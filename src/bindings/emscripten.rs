	...
	( ($( $x:expr ),*) $y:expr ) => {
		{
			...
			#[allow(unused_unsafe)]
			unsafe { self::emscripten_asm_const_int(&LOCAL[0] as *const _ as *const u8, $(self::Interop::as_int($x, &mut arena)),*) }
		}
	};
	( $y:expr ) => {
		{
			...
			#[allow(unused_unsafe)]
			unsafe { self::emscripten_asm_const_int(&LOCAL[0] as *const _ as *const u8) }
		}
	};
}
